...
7. **Companion documents (generate alongside the handoff — strongly recommended for unattended runs).** The handoff is the charter; these carry the implementation-level detail so the new AI almost never needs the legacy repo:
   ...
   Extract both via parallel read-only subagents over the legacy repo (routes, migrations, hooks, importers, sass/css variables, layout partials, every list component's thead). Route each subagent by model tier — see `AGENT_RULES.md` → Subagent Model-Tier Routing (haiku for extraction, sonnet for cross-consistency checks).
